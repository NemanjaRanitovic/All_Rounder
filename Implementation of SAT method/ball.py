...
class ball_object:
    ...
    def is_colliding(self, polygon):
        for i in range(len(polygon.space_vertices)):
            ...
            self_span = span_along_line_1_vertex(v0, normal, self)
...
def span_along_line_1_vertex(vertex, vector, shape):
    span = [math.inf, -math.inf]
    v_vector = [shape.space_vertices[0] - vertex[0], shape.space_vertices[1] - vertex[1]]
    d_p = dot_product(vector, v_vector)
    span[0] = min(span[0], d_p)
    span[1] = max(span[1], d_p)
    return span
